...
<<dataconst_prep, eval=FALSE, echo=FALSE, warning=FALSE>>=
...
compare_altmet_prov <- function(doi, isaddifnot=FALSE, fileout=NULL){
  # plos
...
write.csv(mydf, "~/github/sac/isqaltms/alldatout_2.csv", row.names=FALSE)

alldat <- l_ply(as.character(res[,1]), compare_altmet_prov_safe, isaddifnot=TRUE, fileout="~/github/sac/isqaltms/alldatout_2.csv", .progress="text")
@

<<dataconst_plot, eval=FALSE, echo=FALSE, warning=FALSE, fig.cap="Distribution of absolute differences in least and greatest value of each of seven different altmetrics on a set of 308 DOIs from Altmetric, ImpactStory, and PLoS ALM. Values were log10 transformed to improve comprehension.">>=
library(ggplot2, quietly=TRUE)
library(reshape2, quietly=TRUE)

dat <- read.csv("~/github/sac/isqaltms/alldatout.csv")[-1,]

dat_split <- split(dat, f=dat$doi)
...
dat_split_df_ <- ddply(dat_split_df, .(doi), numcolwise(calcdiff))
dat_split_df_melt <- melt(dat_split_df_)
...
<<dataconst2, eval=FALSE, echo=FALSE, warning=FALSE, fig.cap="A comparison of seven different altmetrics on a set of 20 DOIs from Altmetric, ImpactStory, and PLoS.">>=
...
dois <- c('10.1371/journal.pone.0018657','10.1371/journal.pone.0058869',
          ...
          '10.1371/journal.pone.0052988','10.1371/journal.pbio.1001536',
          '10.1371/journal.pbio.1001537','10.1371/journal.pbio.1001127',
...
# Get the metrics, and make a data.frame
alldat <- ldply(dois, compare_altmet_prov_safe, isaddifnot=TRUE)
...
<<callacrossprov, eval=FALSE, echo=FALSE, results='asis'>>=
library(xtable) 
out <- compare_altmet_prov(doi='10.1371/journal.pone.0018657')
out <- data.frame(provider=out[,1], numcolwise(round, digits=0)(out), date_updated=out[,11])
xtable(out, caption="Example of combining results across three data providers.")
@
